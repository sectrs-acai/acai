...
if [ $DEBARCH == "riscv64" ]; then
    DEBOOTSTRAP_PARAMS="--no-check-gpg --keyring /usr/share/keyrings/debian-ports-archive-keyring.gpg --exclude firmware-atheros $DEBOOTSTRAP_PARAMS http://deb.debian.org/debian-ports"
fi
...
echo 'host0   /mnt    9p      trans=virtio,version=9p2000.L   0 0' | sudo tee -a $DIR/etc/fstab
